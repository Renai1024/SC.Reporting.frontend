    ...
    >
      ...
      >
        <el-row :gutter="15">
          ...
          <el-col :xs="24" :sm="12">
            <el-form-item label="转出车间" prop="fromWorkshop">
              <el-autocomplete
                  ...
                  placeholder="请输入或搜索转出车间"
                  :fetch-suggestions="queryWorkshopSearch"
                  @select="handleWorkshopSelect"
                  clearable
                  class="workshop-autocomplete"
                  @input="handleWorkshopInput"
              >
                <template #default="{ item }">
                  <div>{{ item.code }} - {{ item.name }}</div>
                </template>
          ...
          <el-col :xs="24" :sm="12">
            <el-form-item label="备注" prop="transferReason">
...
const transferRules = {
  ...
  transferPerson: [
    {required: true, message: "请选择转序人员", trigger: "change"}
  ]
};
...
const receiveRules = {
  ...
  receiverPerson: [
    {required: true, message: "请选择收料人员", trigger: "change"}
  ],
  confirmTime: [
...
// 当前激活的自动完成字段（用于区分是哪个输入框）
const currentAutocompleteField = ref('');
const currentAutocompleteForm = ref('');

// 车间搜索结果缓存
const workshopSearchCache = ref({});
// 员工搜索结果缓存
const workerSearchCache = ref({});
// 物料搜索结果缓存
...
/** 车间搜索方法（用于el-autocomplete） */
const queryWorkshopSearch = debounce(async (queryString, cb) => {
  if (!queryString) {
    cb([]);
    return;
  }
  ...
  if (workshopSearchCache.value[cacheKey]) {
    cb(workshopSearchCache.value[cacheKey]);
    return;
  }
  ...
  try {
    ...
    const formattedData = data.map(item => ({
      ...item,
      value: item.name || item.workshopName || '', // 用于显示的值
      code: item.code || item.workshopCode || '',
      name: item.name || item.workshopName || ''
    }));

    // 缓存结果
    workshopSearchCache.value[cacheKey] = formattedData;

    cb(formattedData);
  } catch (error) {
    console.error('车间搜索失败:', error);
    cb([]);
  }
}, 300);
...
const queryWorkerSearch = debounce(async (queryString, cb) => {
  ...
  // 检查缓存
  const cacheKey = queryString.trim().toLowerCase();
  if (workerSearchCache.value[cacheKey]) {
    cb(workerSearchCache.value[cacheKey]);
    return;
  }
  ...
  try {
    ...
    const formattedData = data.map(item => ({
      ...item,
      value: item.name || item.workerName || '', // 用于显示的值
      code: item.code || item.workerCode || '',
      name: item.name || item.workerName || ''
    }));

    // 缓存结果
    workerSearchCache.value[cacheKey] = formattedData;
...
const queryMaterialSearch = debounce(async (queryString, cb) => {
  ...
  // 检查缓存
  const cacheKey = queryString.trim().toLowerCase();
  if (materialSearchCache.value[cacheKey]) {
    cb(materialSearchCache.value[cacheKey]);
    return;
  }
  ...
  try {
    ...
    // 缓存结果
    materialSearchCache.value[cacheKey] = formattedData;
...
/** 选择车间 */
function handleWorkshopSelect(item) {
  // 根据当前操作的表单类型和字段更新对应的表单
  const selectedName = item.name || item.workshopName || '';
...
/** 选择员工 */
function handleWorkerSelect(item) {
...
function handleMaterialSelect(item) {
  ...
  if (transferFormRef.value) {
    // 如果需要，可以同时保存物料名称
    // transferForm.value.materialName = selectedName;
